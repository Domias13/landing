...
function calculateResults() {
    let score = 50;
    if (answers['q7'] === 'no_contact') score += 10;
    if (answers['q9'] === 'no_cheating') score += 15;
    if (answers['q10'] === 'neutral') score += 8;
    if (answers['q11'] === 'no_contact') score += 10;
    if (answers['q14'] >= 4) score += 3;
    score = Math.min(100, Math.max(0, score));
    let category = 'low';
    if (score >= 90) category = 'excellent';
    else if (score >= 75) category = 'good';
    else if (score >= 60) category = 'warning';
...
function showResult(score, category) {
    ...
    const descriptions = {'excellent': 'Всё выглядит перспективно! Высока вероятность воссоединения.', 'good': 'Ситуация не идеальна, но вернуть отношения реально.', 'warning': 'У вас ещё есть возможность, но нужно действовать быстро.', 'low': 'Шансы низки. Возможно, стоит сосредоточиться на себе.'};
